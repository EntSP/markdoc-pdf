...
| `{% media src="…" /%}` | ✅ Works | Plus `alt` / `title` / `caption` / `kind` / `side` / `width`; `{% img %}` is an alias. `title` acts as a caption / alt fallback only when the style sets `image_title_fallback` |
