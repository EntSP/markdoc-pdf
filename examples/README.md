...
| `table-forms.style.toml` (+ `table-forms.mdoc`) | The table *forms* this renderer supports: a plain pipe table, a header-less table (blank header row dropped), a header column (`{% table header_column=true %}`), and per-column text alignment via CommonMark `:---` / `:-:` / `---:` delimiter markers. (The Markdoc list-syntax `* cell` / `---` table is not supported.) |
