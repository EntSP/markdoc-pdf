...
| `columns.mdoc` | Side-by-side layout with `{% columns %}` — equal-width columns of images (or any blocks); each column is a list item (`*`) or a blank-line-separated block, `gap` sets the spacing, and each column may carry a per-column `{% caption %}` (loose-list form). Images now also render inside ordinary table cells. Render with `--assets-root examples`. |
