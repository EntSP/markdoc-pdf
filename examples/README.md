...
| `columns-and-grid.mdoc` | The two multi-column layout primitives. `{% columns %}` — a fixed row of equal or uneven (`widths="2 1"` / `widths=[2, 1]`) columns of images (or any blocks); each column is a list item (`*`) or a blank-line-separated block, `gap` sets the spacing, `align="center"` centres each cell's content, `background="#…"` paints a panel behind the row, and a per-column `{% caption %}` (loose-list form) can sit above or below (`position="below"`) and be tinted with `color="#…"` (or the document-wide `caption_color` style field). A `widths="2 1 2"` row of text / image / text (+ `figure.svg`) centres a figure with independent commentary flanking it on both sides. `{% grid %}` — the same cells *reflow* into as many equal columns as fit at `min` points wide (default 120), wrapping into rows (the print analogue of CSS `repeat(auto-fill, minmax(min, 1fr))`); each row is atomic, so a figure never splits from its caption across a page. Images also render inside ordinary table cells. Render with `--assets-root examples`. |
| `swatch.mdoc` | Block colour bars via `{% swatch %}` — solid (`color="#…"`) or a linear `gradient="[Ndeg,] stop, stop, …"` (evenly spaced stops unless one carries an `NN%` position; `transparent` allowed), sized by `height` / `radius`. The block sibling of the inline `{% color %}`, rendered as a synthesised SVG so gradients need no assets. Shown standalone and as an indicator key — a `{% swatch %}` bar over a bold title and a grey description in each `{% grid %}` cell. Also `{% chip %}`, the inline sibling: a small colour dot (or `shape="square"`) that flows in running text, handy as a compact legend. |
| `qr.mdoc` | QR codes via `{% qr %}` — encodes `value`, which can be a URL, arbitrary text, or a frontmatter field read as `$markdoc.frontmatter.documentNumber`. `size` (points), `ecl` (error correction: `low` / `medium` / `quartile` / `high`), `color` / `background`, `quiet_zone` (modules), and `align`. Drawn as crisp vector (a synthesised SVG). Covers the document-number-on-the-last-page pattern for production scanning. |
| `document-history.mdoc` | The `{% document-history /%}` tag — a revision table drawn from the `documentHistory` frontmatter (`{version, date, description}` entries), so the change log is authored once in metadata and rendered consistently. Shows the default *Document history* heading, a `title=` override, and (in the prose) `title=""` for the table alone. Renders with the default style. |
| `status-lights.mdoc` | A worked status-light legend built from the primitives — a `{% grid %}` of `{% swatch %}` bars (solid colours plus pulsing / blinking / fading gradients) over bold labels and grey `{% color %}` descriptions. Shows how `{% grid %}` and `{% swatch %}` compose into a real, reflowing status-indicator key. |
| `book/` (directory) | A composed multi-file manual. The root `book/book.mdoc` carries a Flux `sections` manifest in its frontmatter, which stitches the chapter files (`intro`, `safety/*`, `specifications`, `operation`, `reference`) into one PDF, sharing the root's title / version / document number. Doubles as a full content showcase: cover, ToC, auto-numbered headings, callouts, blockquotes, every list kind, striped and list-syntax tables, captioned figures, image thumbnails, a figure grid, swatches / chips, columns, a highlighted code block, a footnote, a `{% float %}`, and QR codes — including the last-page document-number stamp. Render with `--input examples/book/book.mdoc --style examples/book/book.style.toml --assets-root examples`. |
...
| `image-formats.mdoc` (+ `sample.jpg` / `sample.gif` / `sample.webp`) | The supported raster image formats, one labelled tile each: JPEG, GIF (static — an animated GIF renders its first frame), and WebP. The format is detected from the file's bytes, not its extension. PNG and SVG render identically and appear in the other examples. Render with `--assets-root examples`. |
| `fonts.mdoc` (+ `fonts.style.toml`) | **A different typeface for each part of one document.** Serif body, sans h1–h3, monospace code, plus independently-set ToC, running header and footer, and a per-kind callout font. `h4`–`h6` and the `warning` callout deliberately set nothing so the page shows inheritance and override side by side: an element's own `font_families` wins, otherwise `body_font_families`, otherwise the bundled Noto set. Every list is a fallback chain (Liberation → DejaVu → Noto), and families that aren't installed are skipped silently, so the contrast survives on a machine missing the first choice. Render with `--style examples/fonts.style.toml`. |
| `scripts.mdoc` | Non-Latin and **bidirectional** text. Fonts are chosen per character from a fallback list, so one document mixes writing systems freely: a greetings table plus paragraphs in Chinese / Japanese / Korean (CJK), Arabic and Hebrew (right-to-left — shaped into joined forms and reordered, including an RTL phrase inside an LTR sentence), Devanagari (Hindi), Thai, Greek and Cyrillic. Needs the matching Noto fonts installed (`Noto Sans CJK`, `Noto Sans Arabic`, `Noto Sans Hebrew`, `Noto Sans Devanagari`, `Noto Sans Thai`); any that are missing fall through to the next family. Renders with the default style. |
